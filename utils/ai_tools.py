...
async def generate_horde_logo_set(name: str, count: int = 4) -> list[str]:
    styles = [
        "minimal vector logo, clean lines, white background",
        "3d glossy emblem logo, dramatic lighting, dark background",
        "modern startup brandmark logo, flat style, high contrast",
        "futuristic neon logo icon, cyber style, premium design",
    ]
    out: list[str] = []
    for style in styles[:max(1, min(count, 4))]:
        prompt = f"{name} {style}"
        url = await generate_horde_image(prompt, width=768, height=768, steps=24)
        if url.startswith("http"):
            out.append(url)
    return out


async def transcribe_audio_free(audio_bytes: bytes) -> str:
    # Local model first (faster-whisper) for reliability.
    try:
        model = WhisperModel("tiny", device="cpu", compute_type="int8")
        tmp = io.BytesIO(audio_bytes)
        with open("/tmp/cypherus_transcribe.wav", "wb") as f:
            f.write(tmp.read())
        segments, _ = model.transcribe("/tmp/cypherus_transcribe.wav", vad_filter=True)
        merged = " ".join(seg.text.strip() for seg in segments if seg.text).strip()
        if merged:
            return merged
    except Exception:
        pass
    # First try a local-free recognizer path (Google Web Speech via SpeechRecognition).
    # Caller should pass WAV PCM bytes for best results.
    try:
    ...
    # Fallback to free HF endpoints if local recognition path fails.
    try:
        async with httpx.AsyncClient(timeout=90) as client:
            last_err = None
            for ep in HF_WHISPER_ENDPOINTS:
                res = await client.post(
                    ep,
                ...
                if isinstance(data, dict):
                    ...
                    if data.get("error"):
                        ...
                        continue
            if last_err:
                return f"Transcription failed: {last_err}"
            return "Transcription unavailable right now. Try again later."
    except Exception as exc:
