    ...
    try:
        ...
        if upd_file.exists():
            ...
            if upd.get("pending_notify"):
                ...
                pip_info = "Packages installed/checked successfully." if int(upd.get("pip_code", 1)) == 0 else "Package install reported errors."
                msg = (
                    "🔄 Cypherus restart report\n"
                    f"{status}\n"
                    f"{pip_info}\n"
                    f"git exit: {upd.get('git_code')} | pip exit: {upd.get('pip_code')}\n"
