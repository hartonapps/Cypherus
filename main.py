            ...
            if cmd in {"help", "menu"}:
                ...
                def _song():
                    ...
                    with yt_dlp.YoutubeDL(opts) as y:
                        info = y.extract_info(f"ytsearch10:{arg}", download=False) or {}
                ...
                if not arg or "spotify.com/track/" not in arg:
                ...
                def _spotify_prepare_and_download():
                    ...
                    with yt_dlp.YoutubeDL(base_opts) as y:
                        info = y.extract_info(arg.strip(), download=False) or {}
                    title = info.get("title") or "Unknown title"
                    source_url = info.get("webpage_url") or arg.strip()
                    dl_opts = {
                        "format": "bestaudio/best",
                        "outtmpl": str(outdir / "%(title).70s.%(ext)s"),
                        "postprocessors": [{"key": "FFmpegExtractAudio", "preferredcodec": "mp3", "preferredquality": "192"}],
